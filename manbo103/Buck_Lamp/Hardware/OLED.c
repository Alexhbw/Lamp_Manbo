...
#define OLED_I2C_ADDR  0x3C << 1   // HAL 需要左移 1 位
...
void OLED_WriteCommand(uint8_t cmd)
{
    ...
    HAL_I2C_Master_Transmit(&hi2c1, OLED_I2C_ADDR, buf, 2, OLED_TIMEOUT_MS);
}

// 连续写数据：控制字节0x40 + N字节数据
void OLED_WriteData(uint8_t *data, uint16_t count)
{
    // 一次最多发 (1 + 128) 这类长度最常见；你也可以分块发
    while (count)
    {
        uint16_t chunk = (count > 128) ? 128 : count;

        uint8_t buf[1 + 128];
        buf[0] = 0x40;
        for (uint16_t i = 0; i < chunk; i++) buf[1 + i] = data[i];

        HAL_I2C_Master_Transmit(&hi2c1, OLED_I2C_ADDR, buf, 1 + chunk, OLED_TIMEOUT_MS);
...
void OLED_Init(void)
{
    HAL_Delay(100);   // 等OLED上电稳定（比你原来for循环更靠谱）
